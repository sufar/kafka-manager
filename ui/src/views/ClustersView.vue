          ...
          <div class="flex items-center gap-2 min-w-0 flex-1">
            ...
            <div class="min-w-0">
              ...
              <div class="flex items-center gap-2 text-[10px] text-base-content/60">
                <span v-if="getGroupName(cluster.group_id)" class="badge badge-ghost badge-xs">
                  {{ getGroupName(cluster.group_id) }}
                </span>
                <span>{{ t.clusters.createdDate }} {{ formatDate(cluster.created_at) }}</span>
              </div>
            </div>
          </div>
...
function getGroupName(groupId: number | null | undefined): string {
  if (!groupId) return '';
  const group = clusterStore.groups.find(g => g.id === groupId);
  return group?.name || '';
}
