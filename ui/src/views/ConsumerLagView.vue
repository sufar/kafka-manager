  ...
  <div class="h-full flex flex-col overflow-auto bg-base-100">
    ...
    <div class="flex items-center justify-between p-3 border-b border-base-300 bg-base-200">
      <div class="flex items-center gap-2">
        <button class="btn btn-square btn-ghost btn-xs" @click="goBack">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
            <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5 3 12m0 0 7.5-7.5M3 12h18" />
          </svg>
        </button>
        <div>
          <h1 class="text-base font-semibold text-base-content">{{ topicName }}</h1>
          <p class="text-xs text-base-content/60">{{ t.consumerLag.cluster }}: {{ clusterName }}</p>
        </div>
      </div>
      <div class="flex items-center gap-2">
        <button class="btn btn-ghost btn-xs" @click="refreshData" :disabled="loading">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-3.5 h-3.5" :class="{ 'animate-spin': loading }">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-3 p-3">
      <div class="stat bg-base-200 rounded-box p-3">
        <div class="stat-title text-base-content/60 text-xs">{{ t.consumerLag.totalLag }}</div>
        <div class="stat-value text-base" :class="{ 'text-error': totalLag > 10000, 'text-warning': totalLag > 1000 && totalLag <= 10000 }">
          {{ formatNumber(totalLag) }}
        </div>
        <div class="stat-desc text-base-content/60 text-xs">{{ t.consumerLag.messages }}</div>
      </div>
      <div class="stat bg-base-200 rounded-box p-3">
        <div class="stat-title text-base-content/60 text-xs">{{ t.consumerLag.consumerGroups }}</div>
        <div class="stat-value text-base text-primary">{{ consumerGroups.length }}</div>
        <div class="stat-desc text-base-content/60 text-xs">{{ t.consumerLag.groups }}</div>
      </div>
      <div class="stat bg-base-200 rounded-box p-3">
        <div class="stat-title text-base-content/60 text-xs">{{ t.consumerLag.partitions }}</div>
        <div class="stat-value text-base text-secondary">{{ partitionCount }}</div>
        <div class="stat-desc text-base-content/60 text-xs">{{ t.consumerLag.partitions }}</div>
      </div>
      <div class="stat bg-base-200 rounded-box p-3">
        <div class="stat-title text-base-content/60 text-xs">{{ t.consumerLag.maxLagGroup }}</div>
        <div class="stat-value text-base truncate" :title="maxLagGroup">
          {{ maxLagGroup || '-' }}
        </div>
        <div class="stat-desc text-base-content/60 text-xs">{{ t.consumerLag.groups }}</div>
      </div>
    ...
    <div class="p-3">
      <div class="card bg-base-200 rounded-box border border-base-300">
        <div class="card-body p-3">
          <h3 class="card-title text-xs">{{ t.consumerLag.lagTrend }}</h3>
          <div v-if="loading" class="flex items-center justify-center h-48">
            <span class="loading loading-spinner loading-sm"></span>
            <span class="ml-2 text-xs text-base-content/60">{{ t.consumerLag.loadingData }}</span>
          </div>
          <div v-else-if="chartData.length === 0" class="flex items-center justify-center h-48">
            <span class="text-xs text-base-content/60">{{ t.consumerLag.noHistoricalData }}</span>
          </div>
          <div v-else class="chart-wrapper" style="height: 220px;">
    ...
    <div class="p-3">
      <div class="card bg-base-200 rounded-box border border-base-300">
        <div class="card-body p-3">
          <h3 class="card-title text-xs">{{ t.consumerLag.consumerGroupDetails }}</h3>
          <div class="overflow-x-auto">
            <table class="table table-zebra w-full text-xs">
              <thead>
                <tr>
                  <th class="font-medium text-base-content/60 text-xs">{{ t.consumerLag.consumerGroups }}</th>
                  <th class="text-right font-medium text-base-content/60 text-xs">{{ t.consumerLag.totalLag }}</th>
                  <th class="font-medium text-base-content/60 text-xs">{{ t.consumerLag.partitionDetails }}</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="group in consumerGroups" :key="group.name">
                  <td class="font-medium text-xs">{{ group.name }}</td>
                  <td class="text-right">
                ...
                <tr v-if="consumerGroups.length === 0">
                  <td colspan="3" class="py-4 text-center text-base-content/60 text-xs">
