<template>
  <div class="p-3">
    <div class="flex justify-between items-center mb-4">
      ...
      <button v-if="selectedGroup" class="btn btn-xs btn-ghost" @click="clearSelectedGroup">
    ...
    <!-- No cluster selected -->
    <div v-if="!clusterParam && selectedClusterIds.length === 0" class="alert alert-info py-2">
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" class="stroke-current shrink-0 w-5 h-5">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
      </svg>
      <span class="text-sm">{{ t.consumerGroups.selectCluster }}</span>
    </div>
  ...
  <!-- Single cluster empty state -->
    ...
    <Teleport to="body">
      <dialog open v-if="resetOffsetModalOpen" class="modal modal-open">
        <div class="modal-box">
          <button class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2" @click="closeResetOffsetModal">
