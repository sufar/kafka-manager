<template>
  <div class="p-4 sm:p-6">
    ...
    <div class="mb-6 sm:mb-8">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-base-content/60 mt-2 text-sm sm:text-lg">{{ t.dashboard.description }}</p>
        </div>
        <button class="btn btn-primary btn-sm sm:btn-md gap-2 flex-shrink-0" @click="refreshAll">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 sm:w-5 sm:h-5" :class="{ 'animate-spin': refreshing }">
    ...
    <template v-else-if="clusters.length > 0">
      <!-- Stats Overview - Use daisyUI stats responsive -->
      <div class="stats stats-vertical lg:stats-horizontal shadow w-full mb-6 sm:mb-8 glass-strong gradient-border overflow-x-auto">
        <div class="stat glass p-2 sm:p-4">
          <div class="stat-figure text-primary glow-primary">
            <div class="p-2 sm:p-3 rounded-xl bg-primary/10">
          ...
          <div class="stat-title text-xs sm:text-sm">{{ t.dashboard.totalClusters }}</div>
          <div class="stat-value text-primary text-2xl sm:text-3xl">{{ totalStats.totalClusters }}</div>
          <div class="stat-desc text-xs sm:text-sm">
            ...
            <span v-if="totalStats.totalClusters - totalStats.healthyClusters > 0" class="text-error ml-1 sm:ml-2 text-xs sm:text-sm">
        ...
        <div class="stat glass p-2 sm:p-4">
          <div class="stat-figure text-secondary glow-secondary">
            <div class="p-2 sm:p-3 rounded-xl bg-secondary/10">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 sm:w-8 sm:h-8">
                <path stroke-linecap="round" stroke-linejoin="round" d="M20.25 6.375c0 2.278-3.694 4.125-8.25 4.125S3.75 8.653 3.75 6.375m16.5 0c0-2.278-3.694-4.125-8.25-4.125S3.75 4.097 3.75 6.375m16.5 0v11.25c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125V6.375m16.5 0v3.75m-16.5-3.75v3.75m16.5 0v3.75C20.25 16.153 16.556 18 12 18s-8.25-1.847-8.25-4.125v-3.75m16.5 0c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125" />
              </svg>
            </div>
          </div>
          <div class="stat-title text-xs sm:text-sm">{{ t.dashboard.totalTopics }}</div>
          <div class="stat-value text-secondary text-2xl sm:text-3xl">{{ totalStats.totalTopics }}</div>
          <div class="stat-desc text-xs sm:text-sm">{{ t.dashboard.partitions }}/{{ t.dashboard.topics }} avg</div>
        </div>

        <div class="stat glass p-2 sm:p-4">
          <div class="stat-figure text-accent">
            <div class="p-2 sm:p-3 rounded-xl bg-accent/10">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 sm:w-8 sm:h-8">
          ...
          <div class="stat-title text-xs sm:text-sm">{{ t.dashboard.totalPartitions }}</div>
          <div class="stat-value text-accent text-2xl sm:text-3xl">{{ formatNumber(totalStats.totalPartitions) }}</div>
          <div class="stat-desc text-xs sm:text-sm">{{ t.dashboard.partitionsPerTopic }}</div>
        </div>

        <div class="stat glass p-2 sm:p-4">
          <div class="stat-figure text-warning">
            <div class="p-2 sm:p-3 rounded-xl bg-warning/10">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 sm:w-8 sm:h-8">
          ...
          <div class="stat-title text-xs sm:text-sm">{{ t.dashboard.consumerGroups }}</div>
          <div class="stat-value text-warning text-2xl sm:text-3xl">{{ totalStats.totalConsumerGroups }}</div>
          <div class="stat-desc text-xs sm:text-sm">{{ t.dashboard.totalLag }}: {{ formatLag(totalStats.totalLag) }}</div>
        </div>
      </div>
      ...
      <div class="mb-6 sm:mb-8">
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-lg sm:text-xl font-semibold text-gradient">{{ t.dashboard.clusters }}</h2>
