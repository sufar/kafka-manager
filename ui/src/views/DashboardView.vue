<template>
  <div class="p-6 relative overflow-hidden">
    ...
    <div class="mb-8 relative">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-base-content/60 mt-2 text-lg">Overview of your Kafka clusters</p>
        </div>
        <button class="btn btn-primary gap-2 glow-primary" @click="refreshAll">
    ...
    <template v-else-if="clusters.length > 0">
      ...
      <div class="stats stats-vertical lg:stats-horizontal shadow w-full mb-8 glass-strong gradient-border">
        <div class="stat glass">
          ...
          <div class="stat-desc">
            <span class="text-success">{{ totalStats.healthyClusters }} healthy</span>
            <span v-if="totalStats.totalClusters - totalStats.healthyClusters > 0" class="text-error ml-2">
              {{ totalStats.totalClusters - totalStats.healthyClusters }} unhealthy
            </span>
          </div>
        </div>
        ...
        <div class="stat glass">
          ...
          <div class="stat-desc">Across all clusters</div>
        </div>
        ...
        <div class="stat glass">
          ...
          <div class="stat-title">Total Partitions</div>
          <div class="stat-value text-accent">{{ formatNumber(totalStats.totalPartitions) }}</div>
          <div class="stat-desc">{{ avgPartitionsPerTopic }} partitions/topic avg</div>
        </div>
        ...
        <div class="stat glass">
          ...
          <div class="stat-desc">Total lag: {{ formatLag(totalStats.totalLag) }}</div>
        </div>
      </div>
      ...
      <div class="mb-8">
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-xl font-semibold text-gradient">Clusters</h2>
          ...
          >
            <div class="card-body p-4">
              ...
              <div class="flex items-center justify-between mb-2">
                ...
                >
                  {{ getClusterHealth(cluster.name)?.healthy ? 'Connected' : 'Disconnected' }}
                </div>
              </div>
              ...
              <div v-if="getClusterHealth(cluster.name)?.stats" class="grid grid-cols-3 gap-3 mb-3">
                <div class="text-center">
                  <div class="text-xs uppercase tracking-wide text-base-content/50">Topics</div>
                  <div class="text-lg font-semibold">{{ getClusterHealth(cluster.name)?.stats?.topic_count || 0 }}</div>
                </div>
                <div class="text-center">
                  <div class="text-xs uppercase tracking-wide text-base-content/50">Partitions</div>
                  <div class="text-lg font-semibold">{{ getClusterHealth(cluster.name)?.stats?.partition_count || 0 }}</div>
                </div>
                <div class="text-center">
                  <div class="text-xs uppercase tracking-wide text-base-content/50">Lag</div>
              ...
              <div v-if="getClusterHealth(cluster.name)?.lastChecked" class="text-xs text-base-content/50">
                Last checked: {{ formatRelativeTime(getClusterHealth(cluster.name)?.lastChecked) }}
              </div>
      ...
      <div class="card bg-base-100 shadow-md mb-8">
        <div class="card-body">
          <div class="flex items-center justify-between mb-4">
            <h2 class="text-lg font-semibold">Consumer Lag Overview</h2>
          ...
          <div class="border-2 border-dashed border-base-300 rounded-box min-h-[240px] flex items-center justify-center">
            <div class="text-center">
              ...
              <p class="text-base-content/60 mt-4">Chart integration coming soon</p>
              <p class="text-base-content/40 text-sm mt-2">Total lag: {{ formatLag(totalStats.totalLag) }} across all consumer groups</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-else class="flex flex-col items-center justify-center py-16 text-center">
      ...
      <h3 class="text-xl font-semibold mb-2">No Clusters Connected</h3>
      <p class="text-base-content/60 mb-6">Get started by adding your first Kafka cluster</p>
