    ...
    <div class="flex items-center gap-0.5 flex-shrink-0">
      ...
      <div v-if="isMobile" class="dropdown dropdown-end md:hidden">
        ...
        <ul tabindex="0" class="dropdown-content menu menu-sm bg-base-100 rounded-box shadow-xl z-50 w-32 mt-2">
          ...
          <li>
            <a @click="$emit('start-tour')">
              <span class="text-xs">{{ t.layout.help || 'Help' }}</span>
            </a>
          </li>
          <li>
          ...
          <li>
            <a @click="goToSchemaRegistry">
              <span class="text-xs">{{ t.navigator.schemaRegistry }}</span>
            </a>
          </li>
        </ul>
      </div>
      ...
      <div class="hidden md:flex items-center gap-0.5">
        ...
        <!-- Settings Button -->
        <router-link to="/settings" class="btn btn-ghost btn-circle btn-xs h-6 w-6 min-h-0" :title="t.layout.settings" data-tour="settings">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-3.5 h-3.5">
...
// 跳转到 Schema Registry 页面（优先使用当前选中的集群）
function goToSchemaRegistry() {
  const clusterName = clusterStore.selectedCluster?.name || clusterStore.clusters[0]?.name;
  if (clusterName) {
    router.push({ path: '/schema-registry', query: { cluster: clusterName } });
  } else {
    router.push({ path: '/schema-registry' });
  }
}
