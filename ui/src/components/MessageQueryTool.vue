  ...
  <div class="message-query-tool flex-1 flex flex-col min-h-0 relative">
    ...
    <div class="toolbar flex flex-wrap items-center gap-1.5 p-1.5 border-b border-base-300 bg-base-100">
      ...
      <div class="flex-1 min-w-[120px] relative" data-tour="messages-search">
        ...
        <!-- 搜索范围选择器（绝对定位在输入框内左侧） -->
        <select v-model="searchIn" class="absolute left-1 top-1/2 -translate-y-1/2 select select-ghost select-xs w-16 p-0 h-auto text-xs bg-transparent border-none focus:outline-none cursor-pointer text-base-content/70 hover:text-base-content">
    ...
    <div class="status-bar flex items-center justify-between px-3 py-1 text-xs border-b border-base-300 bg-base-200/50">
      <div class="flex items-center gap-2">
        ...
        <div class="flex items-center gap-0.5">
          <!-- 导出提示 -->
          <span v-if="messages.length > 0" class="tooltip tooltip-right" :data-tip="t.messages.exportTip">
          ...
          <!-- 导出消息 -->
          <button class="btn btn-ghost btn-xs" :disabled="messages.length === 0" @click="exportMessages" :title="t.messages.exportMessages" data-tour="messages-export">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-3 h-3">
              <path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5M16.5 12 12 16.5m0 0L7.5 12m4.5 4.5V3" />
            </svg>
          </button>
          <!-- 发送历史 -->
          <button class="btn btn-ghost btn-xs" :disabled="!selectedCluster || !selectedTopic" :title="t.sentMessageHistory?.title || '发送历史'" @click="toggleHistory" data-tour="messages-history">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-3 h-3">
              <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0Z" />
            </svg>
          </button>
          <!-- 消费者组 -->
          <button class="btn btn-ghost btn-xs" :disabled="!selectedCluster || !selectedTopic" :title="t.topicConsumerGroups?.title || '消费者组'" @click="viewTopicConsumerGroups" data-tour="messages-consumer-groups">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-3 h-3">
          ...
          <!-- 删除主题 -->
          <button class="btn btn-ghost btn-xs text-error" :disabled="!selectedCluster || !selectedTopic" :title="t.messages?.deleteTopic || '删除主题'" @click="handleDeleteTopic" data-tour="messages-delete-topic">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-3 h-3">
