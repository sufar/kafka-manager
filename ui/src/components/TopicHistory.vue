  ...
  <div class="topic-history">
    ...
    <div class="relative mb-2">
      <input
        ...
        :placeholder="t.history?.searchPlaceholder || '搜索 Cluster、Topic...'"
      />
    ...
    <div class="history-list">
      ...
      <div v-else>
        <div
          ...
          :key="item.id"
          class="history-item"
          @click="navigateToTopic(item.cluster_id, item.topic_name)"
        >
...
const filteredHistories = computed(() => {
  ...
  return histories.value.filter((item) => {
    ...
    return matchCluster || matchTopic;
  });
});
...
// Listen for topic navigation from other components
onMounted(() => {
  loadHistory();

  // Listen for topic view events to auto-record history
  window.addEventListener('navigate-to-topic-from-history', ((e: CustomEvent) => {
    const { clusterId, topicName } = e.detail;
    if (clusterId && topicName) {
      // Record history silently
      apiClient.recordTopicHistory(clusterId, topicName).catch(() => {});
    }
  }) as EventListener);
});
</script>
