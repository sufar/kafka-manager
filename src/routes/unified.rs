...
async fn handle_message_list(state: AppState, body: Value) -> Result<Value> {
    ...
    // 尝试从 Pool 获取 Consumer 进行查询，如果 Pool 不可用则使用临时 Consumer
    let messages = match state.pools.get_consumer_pool(&cluster_id).await {
        Some(pool) => {
            tracing::info!("[MessageQuery] Using consumer pool for cluster {}", cluster_id);
            messages::fetch_messages_with_pool(
                &pool,
                &config.brokers,
                &topic,
                partition,
        ...
        None => {
            tracing::warn!("[MessageQuery] Pool not available for cluster {}, falling back to temp consumer", cluster_id);
            messages::fetch_messages_with_temp_consumer(
                &config.brokers,
                &topic,
                partition,
                offset,
...
async fn handle_message_export(state: AppState, body: Value) -> Result<Value> {
    ...
    let messages = messages::fetch_messages_with_temp_consumer(
        &config.brokers,
        &topic,
        partition,
        offset,
...
async fn handle_connection_metrics(state: AppState, body: Value) -> Result<Value> {
    ...
    // 重新获取 Pool 配置
    let pool_config = state.pools.get_pool_config(&cluster_id).await;
    ...
    // 获取更详细的 Pool 状态
    let consumer_waiting = if let Some(pool) = state.pools.get_consumer_pool(&cluster_id).await {
        pool.status().waiting
    } else {
        0
    };
    ...
    // 配置的 Pool 大小
    let consumer_max_size = pool_config.as_ref().map(|c| c.max_size).unwrap_or(10);
    let consumer_min_size = pool_config.as_ref().map(|c| c.min_size).unwrap_or(5);
    let producer_max_size = pool_config.as_ref().map(|c| c.max_size).unwrap_or(10);
    let producer_min_size = pool_config.as_ref().map(|c| c.min_size).unwrap_or(5);
    ...
    Ok(serde_json::json!({
        ...
        // 当前已使用连接数
        "consumer_pool_used": consumer_pool_size - consumer_pool_available,
        "producer_pool_used": producer_pool_size - producer_pool_available,
        // 等待队列
        "consumer_pool_waiting": consumer_waiting,
        // 配置的 Pool 大小（这才是用户想要看到的）
        "consumer_pool_max_size": consumer_max_size,
        "consumer_pool_min_size": consumer_min_size,
        "producer_pool_max_size": producer_max_size,
        "producer_pool_min_size": producer_min_size,
    }))
}
